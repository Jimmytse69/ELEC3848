...
//pins for right ultasonic sensor
#define echoPinR 29
#define trigPinR 30

//pins for left ultasonic sensor
#define echoPinL 32
#define trigPinL 33

long distance_in_cmR;
long distance_in_cmL;
  ...
  */
  ...
  if (SERIAL.available())
  {
    ...
    if ((firstValue.toInt() > servo_min and firstValue.toInt() < servo_max) and  //Convert them to numbers
      ...
      //window_size = thirdValue.toInt() / 1000;
    }
    window_size = thirdValue.toInt() / 1000; //original from if (){..}
    SERIAL.flush();
...
//ultrasound measurement of distance
//RIGHT
void measure_distanceR() {
  long duration;
  ...
  distance_in_cmR = (duration / 2.0) / 29.1;
}
...
void measure_distanceL() {
  ...
  distance_in_cmL = (duration / 2.0) / 29.1;
}

//wireless charing, Task(2)
void Wireless(){


  if (distance_in_cmL < 8 || distance_in_cmR < 8) {
    STOP();
  }

  else if (distance_in_cmL < 25 || distance_in_cmR < 25) {   //ultrosonic take over
    if (distance_in_cmR - distance_in_cmL >= 2) {
      rotate_1();
      delay(50);
      STOP();
    }

    else if (distance_in_cmL - distance_in_cmR >= 2) {
      rotate_2();
      delay(50);
      STOP();
    }
    
    /*else if ( distance_in_cmR - distance_in_cmL >= 20 ) {
      LEFT_2();
    }
    else if ( distance_in_cmL - distance_in_cmR >= 20 ) {
      RIGHT_2();
    }*/

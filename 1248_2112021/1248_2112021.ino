...
void loop()
{
  ...
  if (distance_in_cmL < 25 || distance_in_cmR < 25) {   //ultrosonic take over
    if ( distance_in_cmR - distance_in_cmL >= 3 ) {
      rotate_1();
      delay(100);
      STOP();
      //delay(100);
    }
    else if ( distance_in_cmL - distance_in_cmR >= 3 ) {
      rotate_2();
      delay(100);
      STOP();
      //delay(100);
    }
    else if ( distance_in_cmL < 9 || distance_in_cmR < 9) {
      /*if (sendVolt() == 0){
        LEFT_2();
        if (distance_in_cmL > 10){
          RIGHT_2();
          if (sendVolt()){
            STOP();
          }
  ...
  /*
    ...
    display.println(window_size);
    //display.setCursor(0, 20);
    //display.println(window_size/1000);    //in terms of k.
    display.display();
  */
